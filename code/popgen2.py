...
# slightly altered by Cristian Ion
...
ap.add_argument("--tri", type=bool,default=False)
ap.add_argument("--sqr", type=bool,default=False)
ap.add_argument("--saw", type=bool,default=False)
ap.add_argument("--test", action="store_true", help=argparse.SUPPRESS)
args = ap.parse_args()

# Triangle wave argument
tri = args.tri

# Square wave argument
sqr = args.sqr
...
### make different waveforms other than sine waves
def make_triangle(key,n=1):
    ...
    # https://www.mathworks.com/help/sltest/ref/triangle.html
    triangle = 2 * np.abs(2 * (t * f - np.floor(1/2 + t * f))) - 1

    return triangle

def make_square(key,n=1):

    f = 440*2**((key-69)/12)
    b = beat_samples * n
    t = np.linspace(0,n,b)

    # https://www.quora.com/What-is-the-equation-of-the-square-wave
    square = np.sign(np.sin(2*np.pi*f*t))

    return square

def make_saw(key,n=1):
...
for c in chord_loop:
    if tri:
        ...
        melody = np.concatenate(list(make_triangle(i + melody_root) for i in notes))

        bass_note = note_to_key_offset(c - 1)
        bass = make_triangle(bass_note + bass_root, n=4)
        ...
        notes = pick_notes(c - 1)
        melody = np.concatenate(list(make_square(i + melody_root) for i in notes))

        bass_note = note_to_key_offset(c - 1)
        bass = make_square(bass_note + bass_root, n=4)
        ...
        notes = pick_notes(c - 1)
        melody = np.concatenate(list(make_saw(i + melody_root) for i in notes))

        bass_note = note_to_key_offset(c - 1)
        bass = make_saw(bass_note + bass_root, n=4)
...
triangle_wave = make_triangle(60, n=1)

# Plot the waveform
plt.figure(figsize=(10, 4))
plt.plot(triangle_wave[:1000])  # Plot first 1000 samples for a clear view
plt.title("Triangle Wave (First 1000 samples)")
plt.xlabel("Samples")
...
sine_wave = make_note(60, n=1)

# Plot the waveform
plt.figure(figsize=(10, 4))
plt.plot(sine_wave[:1000])  # Plot first 1000 samples for a clear view
...
square_wave = make_square(60, n=1)

# Plot the waveform
plt.figure(figsize=(10, 4))
plt.plot(square_wave[:1000])  # Plot first 1000 samples for a clear view
plt.title("Square Wave (First 1000 samples)")
plt.xlabel("Samples")
plt.ylabel("Amplitude")
plt.grid(True)
#plt.show()
...
saw_wave = make_saw(60, n=1)

# Plot the waveform
plt.figure(figsize=(10, 4))
plt.plot(saw_wave[:1000])  # Plot first 1000 samples for a clear view
plt.title("Saw Wave (First 1000 samples)")
plt.xlabel("Samples")
plt.ylabel("Amplitude")
plt.grid(True)
#plt.show()
